...
function RunHackScript_v1(ns, sitrep) {
  if (
    sitrep.servers.some(
      (s) => s.ports.nuked == false
    ) /*|| sitrep.ram.total < 5000*/ &&
    ...
    pid = ns.getRunningScript(
      "/scripts/hack/loop_hack/v1.js",
      "home",
      "xp"
    )
    if (pid == undefined) {
      ns.tprint("INFO: Starting v1.js with params [xp]")
      pid = ns.run("/scripts/hack/loop_hack/v1.js", 1, "xp")
      if (pid == undefined) {
    ...
    // see if v1 already runs
    const processInfo = ns
      .ps()
      .find((p) => p.filename == "scripts/hack/loop_hack/v1.js")
    let top_hack_target = GetTopHackServers(ns, 10)[0].name

    // ns.tprint(JSON.stringify(processInfo))
    let parametersChanged = false
    if (processInfo != null && processInfo.args[0] != top_hack_target) {
      parametersChanged = true
      ns.print("Switching v1.js target from " + processInfo.args[0] + " to " + top_hack_target)
    }
    // if v1 not running, or if target changed
    if (processInfo == null || parametersChanged) {
      if (processInfo != null) {
        ns.tprint("WARN: Killing v1.js " + processInfo.args)
        LogMessage(ns, "WARN: Killing v1.js " + processInfo.args)
        ns.kill(processInfo.pid)
      }
      // ns.tprint("INFO: Starting v1.js with params " + top_hack_target)
      LogMessage(ns, "INFO: Starting v1.js with params " + top_hack_target)
      pid = ns.run("/scripts/hack/loop_hack/v1.js", 1, top_hack_target)
      if (pid == undefined) {
        ns.tprint("FAIL: Failed to start v1.js " + top_hack_target)
      }
    }
  }
}

function RunHackScript_v2(ns, sitrep) {
  // ns.tprint("RunHackScript_v2 start")
  let pid = -1
  if (
    sitrep.servers.some(
      (s) => s.ports.nuked == false
    ...
    pid = ns.getRunningScript(
      "/scripts/hack/loop_hack/v2.js",
      "home",
      "xp"
    )
    if (pid == undefined) {
      ns.tprint("INFO: Starting v2.js with params [xp]")
      pid = ns.run("/scripts/hack/loop_hack/v2.js", 1, "xp")
      if (pid == undefined) {
        ns.tprint("FAIL: Failed to start v2.js with params [xp]")
      }
    }
  } else {
    // Kill XP farming script
    ...
    if (pid != undefined) {
      ns.tprint("INFO: XP goal reached, killing v2.js with params [xp]")
      ns.kill(pid.pid)
      pid = undefined
    }

    // see if v2 already runs
    const processInfo = ns
      .ps()
      .find((p) => p.filename == "scripts/hack/loop_hack/v2.js")

    // if v1 not running, or if target changed
    if (processInfo == null) {
      LogMessage(ns, "INFO: Starting v2.js with params hack")
      pid = ns.run("/scripts/hack/loop_hack/v2.js", 1, "hack")
      if (pid == undefined) {
        ns.tprint("FAIL: Failed to start v2.js " + "hack")
      }
    }
  }
}

function RunHackScript_v3(ns, sitrep) {
  // ns.tprint("RunHackScript_v2 start")
  let pid = -1
  if (
    sitrep.servers.some(
    ...
    pid = ns.getRunningScript(
      "/scripts/hack/loop_hack/v3.js",
      "home",
      "xp"
    )
    if (pid == undefined) {
      ns.tprint("INFO: Starting v3.js with params [xp]")
      pid = ns.run("/scripts/hack/loop_hack/v3.js", 1, "xp")
      if (pid == undefined) {
        ns.tprint("FAIL: Failed to start v3.js with params [xp]")
      }
    }
  } else {
    // Kill XP farming script
    ...
    if (pid != undefined) {
      ns.tprint("INFO: XP goal reached, killing v3.js with params [xp]")
      ns.kill(pid.pid)
      pid = undefined
    }

    // see if v3 already runs
    const processInfo = ns
      .ps()
      .find((p) => p.filename == "scripts/hack/loop_hack/v3.js")

    // if v1 not running, or if target changed
    if (processInfo == null) {
      LogMessage(ns, "INFO: Starting v3.js with params hack")
      pid = ns.run("/scripts/hack/loop_hack/v3.js", 1, "hack")
      if (pid == undefined) {
        ns.tprint("FAIL: Failed to start v3.js " + "hack")
      }
    }
  }
}
