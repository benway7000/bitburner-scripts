...
export async function main(ns) {
	...
	for (let i = 0; i < servers.length; i++) {
		...
		let hacking = hack_report.currentTargets.includes(server.name) ? hack_report.serverStates[server.name].phase.toUpperCase()[0] + ' ' + FormatTime(hack_report.serverStates[server.name].expectedTime) : ''
		...
		let values = [
			...
			{ color: maxRam > 0 ? freeRamColor : 'Grey', text: ' ' + freeRamString.padStart(8) + (maxRam == 0 ? ' ' : '/') + ramString.padStart(8) + ' ' + ramPct.padStart(4) },
			...
			hackable ? { color: prepped ? 'lime' : 'Grey', text: prepped ? 'Yes'.padStart(6) : '-'.padStart(5) } : '',
			weight ? { color: 'white', text: ' ' + (weight).toFixed(0) } : '',
			{ color: 'lime', text: hacking.padStart(9) },
		];
