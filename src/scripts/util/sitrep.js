import { GetAllServers, GetServerPath, HasFormulas } from 'scripts/lib/utils'
...
export async function main(ns) {
  ...
  if (HasFormulas(ns)) {
	...
	ns.write('/data/sitrep.txt', JSON.stringify(report, null, 2), 'w');
	// ns.tprint(JSON.stringify(report));
}

export function GetSitRep(ns) {
	return JSON.parse(ns.read('/data/sitrep.txt'));
}