...
export async function main(ns) {
  ...
  let servers = GetAllServers(ns)
    .filter(
      (s) => ns.getServer(s).hasAdminRights && !s.startsWith("hacknet-node-")
    )
    .sort((a, b) => ns.getServer(b).maxRam - ns.getServer(a).maxRam)
  let details = []
  let totalProcs = 0
  ...
  if (ns.args.length == 0) {
    for (const server of servers) {
      ...
      let entry = [
        ...
        { color: "white", text: ns.formatRam(total, 2).padStart(9) },
        {
          color: pctColor(1 - usedPct / 100),
          text:
            ns.formatRam(used, 2).padStart(9) +
            (usedPct.toFixed(0) + "%").padStart(5),
        },
        {
          color: pctColor(freePct / 100),
          text:
            ns.formatRam(free, 2).padStart(9) +
            (freePct.toFixed(0) + "%").padStart(5),
        },
      ]
      ...
      procs.forEach(function (s) {
        entry.push({
          color: pctColor(1 - s.percent / 100),
          text: s.percent > 0 ? (s.percent.toFixed(0) + "%").padStart(7) : "",
        })
      })
    ...
    let network = servers.filter(
      (s) => s != "home" && !personals.includes(s) /*&& !hashnet.includes(s)*/
    )
    ...
    for (const cat of cats) {
      ...
      for (const server of cat.servers) {
        ...
        for (let proc of serverProcs) {
          ...
          if (match == undefined) procs.push(proc)
          else {
            match.ram += proc.ram
  ...
  for (let i = 0; i < details[0].length; i++) {
    ...
    entry.push({
      color: pctColor(1 - pct / 100),
      text: pct > 0 ? (pct.toFixed(1) + "%").padStart(7) : "",
    })
  }
...
export class MemoryMap {
  constructor(ns, simulateFull = false) {
    ...
    for (var server of servers) {
      ...
      if (server == "home") this.home = so.maxRam
      else if (so.purchasedByPlayer) this.purchased += so.maxRam
      else this.other += so.maxRam
  ...
  SmallestBlock(minSize = 0) {
    ...
    for (const block of this.blocks) {
      ...
      if (available < smallest && available >= minSize) smallest = available
    }
    return smallest
  }
  ...
  BiggestBlock() {
    ...
    for (const block of this.blocks) {
      const available = block.free - block.reserved
      if (available > biggest) biggest = available
    }
    return biggest
  }
...
export function RunScript(
  ...
  while (fired < threads) {
    ...
    if (scriptName.includes("grow") || scriptName.includes("weaken")) {
      let homeBlock = ramMap.HomeBlock()
      if (
        homeBlock != undefined &&
        homeBlock.coreBonus > 1 &&
        threads * ram < homeBlock.free - homeBlock.reserved
      ) {
        if (server == "home") {
    ...
    } else if (!allowPartial && fired < threads) {
      // Couldn't find a block big enough, and we didn't fire all the threads
      // TODO: kill any running pids
      break
    }
  ...
  if (fired != threads) {
    ns.print(
      "ERROR: No server big enough to handle " +
        threads +
        " threads of " +
        scriptName +
        " (fired " +
